...
type LoopbackRoot struct {
	...
	// LoopbackNode.
	NewNode func(rootData *LoopbackRoot, parent *fs.Inode, name string, st *syscall.Stat_t, cli *logic.NotifyServiceClient) fs.InodeEmbedder
}

func (r *LoopbackRoot) newNode(parent *fs.Inode, name string, st *syscall.Stat_t, cli *logic.NotifyServiceClient) fs.InodeEmbedder {
	if r.NewNode != nil {
		return r.NewNode(r, parent, name, st, cli)
	}
	return &LoopbackNode{
		RootData: r,
		cli:      cli,
...
func (n *LoopbackNode) pathRelative(name string) string {
	return filepath.Join(n.Path(nil), name)
}
...
func (n *LoopbackNode) Lookup(ctx context.Context, name string, out *fuse.EntryOut) (*fs.Inode, syscall.Errno) {
	...
	node := n.RootData.newNode(n.EmbeddedInode(), name, &st, n.cli)
...
func (n *LoopbackNode) Mknod(ctx context.Context, name string, mode, rdev uint32, out *fuse.EntryOut) (*fs.Inode, syscall.Errno) {
	...
	n.cli.Mknod(n.pathRelative(name), mode, rdev)
	return ch, 0
}
...
func (n *LoopbackNode) Mkdir(ctx context.Context, name string, mode uint32, out *fuse.EntryOut) (*fs.Inode, syscall.Errno) {
	...
	n.cli.Mkdir(n.pathRelative(name), mode)
	return ch, 0
}
...
func (n *LoopbackNode) Rmdir(ctx context.Context, name string) syscall.Errno {
	...
	err := syscall.Rmdir(p)
	if err == nil {
		n.cli.Rmdir(n.pathRelative(name))
	}
	return fs.ToErrno(err)
}
...
func (n *LoopbackNode) Rename(ctx context.Context, name string, newParent fs.InodeEmbedder, newName string, flags uint32) syscall.Errno {
	...
	if err == nil {
		n.cli.Rename(n.pathRelative(name), newName)
	}
	return fs.ToErrno(err)
}
...
func (n *LoopbackNode) Create(ctx context.Context, name string, flags uint32, mode uint32, out *fuse.EntryOut) (inode *fs.Inode, fh fs.FileHandle, fuseFlags uint32, errno syscall.Errno) {
	...
	node := n.RootData.newNode(n.EmbeddedInode(), name, &st, n.cli)
	ch := n.NewInode(ctx, node, n.RootData.idFromStat(&st))
	lf := NewLoopbackFile(fd, n.cli, n.pathRelative(name))

	out.FromStat(&st)

	n.cli.Create(n.pathRelative(name), mode, flags)
	return ch, lf, 0, 0
}
...
func (n *LoopbackNode) Open(ctx context.Context, flags uint32) (fh fs.FileHandle, fuseFlags uint32, errno syscall.Errno) {
	...
	lf := NewLoopbackFile(f, n.cli, n.Path(n.Root()))
	return lf, 0, 0
}
...
func NewLoopbackRoot(rootPath string, cli *logic.NotifyServiceClient) (fs.InodeEmbedder, error) {
	...
	return root.newNode(nil, "", &st, cli), nil
}
